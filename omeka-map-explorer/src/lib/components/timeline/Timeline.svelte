<script lang="ts">
	...
	function initChartInternal() {
		...
		// Create SVG
		chart = d3.select(svgElement).append('svg').attr('width', '100%').attr('height', height);
		...
		// Create area path
		g.append('path').attr('class', 'area-path').attr('fill', '#4a86e8').attr('fill-opacity', 0.6);
	...
	function updateChartDimensions() {
		...
		currentDateLine.attr('y2', parseInt(height) - margin.top - margin.bottom);
	...
	function updateChart() {
		...
		// Update area path
		chart.select('.area-path').datum(data).attr('d', areaGenerator);
	...
	function handleTimelineInteraction(event: MouseEvent | KeyboardEvent) {
		...
		const clientX =
			event instanceof MouseEvent ? event.clientX : container.left + container.width / 2;
	...
	function updateCurrentDateIndicator(date: Date) {
		...
		chart.select('.current-date-line').attr('x1', xPos).attr('x2', xPos);
	}
		...
		<div class="speed-control">
			...
			<input
				...
				oninput={(e) => {
					...
					if (!isNaN(v)) timeData.playbackSpeed = v;
				}}
			/>
