<script lang="ts">
  ...
  // Props (runes mode)
  let { map, geoJson, data = {}, colorRange = schemeBlues[7], scaleMode = 'log' } = $props<{
    map: any;
    geoJson: GeoJsonData;
  ...
  function generateColorScale(data: Record<string, number>, range: string[], mode: 'log'|'linear'|'quantile') {
    const raw = Object.values(data).filter((v) => Number.isFinite(v) && v >= 0);
    if (raw.length === 0) {
      const scale = scaleQuantize().domain([0, 1]).range(range as string[]);
      return (v: number) => scale(v || 0) as string;
    }

    if (mode === 'quantile') {
  const scale = scaleQuantile().domain(raw).range(range as string[]);
  return (v: number) => scale(v || 0) as string;
    }
  ...
  const scale = scaleQuantize().domain([min, max]).range(range as string[]);
  return (v: number) => scale(transform(v || 0)) as string;
  }

  function computeLegendBins(data: Record<string, number>, steps: number, mode: 'log'|'linear'|'quantile') {
    const raw = Object.values(data).filter((v) => Number.isFinite(v) && v >= 0).sort((a,b)=>a-b);
    if (raw.length === 0) {
      return Array.from({ length: steps }, (_, i) => ({ from: i/steps, to: (i+1)/steps }));
    }
    if (mode === 'quantile') {
  const q = scaleQuantile().domain(raw).range(Array.from({ length: steps }, (_, i) => i));
